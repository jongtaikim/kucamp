...
list($img,$font,$size2,$color,$color2) = $path_infos;

if( !$img || !$font || !$size2 || !$color2){
	echo "404 error";
	exit;
}
...
//$im_=imagecreate($ww,$hh);

$im_ = imagecreatetruecolor($ww, $hh);
imagesavealpha($im_, true);
$trans_colour = imagecolorallocatealpha($im_, 0, 0, 0, 127);
imagefill($im_, 0, 0, $trans_colour);


$back=imagecolorallocate($im_,$bbcolor2_array[0],$bbcolor2_array[1],$bbcolor2_array[2]);

$pen_color2= imagecolorallocate($im_,$ffcolor2_array[0],$ffcolor2_array[1],$ffcolor2_array[2]);

imagefttext($im_,$size,0,$x,$y,$pen_color2,$font_file, $text);
if($bold2){
	imagefttext($im_,$size,0,$x,$y+0.8,$pen_color2,$font_file, $text);
	imagefttext($im_,$size,0,$x,$y-0.8,$pen_color2,$font_file, $text);
	imagefttext($im_,$size,0,$x+0.8,$y,$pen_color2,$font_file, $text);
	imagefttext($im_,$size,0,$x-0.8,$y,$pen_color2,$font_file, $text);
...
$trans_colour = imagecolorallocatealpha($im2_, 0, 0, 0, 127);
imagefill($im2_, 0, 0, $trans_colour);
imagecopyresampled($im2_, $im_, 0, 0, 0, 0, $width, $height, imagesx($im_), imagesy($im_));
...
switch ($img_) {
	case "submenuBg.gif":
		$im2 =   CreateButton_4W($tmp_img ,$im2 ,$im2_ ,$width ,$height,10,20);	
	 break;

	 case "subMenuBg_sub.gif":
		$im2 =   CreateButton_4W($tmp_img ,$im2 ,$im2_ ,$width ,$height,10,15);	
	 break;
	
	
	default:
		$im2 =   CreateButton_2W($tmp_img ,$im2 ,$im2_ ,$width ,$height,1,30);
	 break;
	}
