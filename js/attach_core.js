...
'<td style="font-size:11px"  ><img src="/f.php?text='+icon+title+'&font=yg330&size2=10&bg=F9FCDD&color=#5B5B5B&mk_btn=Y"  align=absmiddle >'+ne_img+'<input type="checkbox" name="'+name+'(str_lock)" onfocus="this.blur()" style="cursor: hand;width:10px;height:10px"  align=absmiddle '+chk+' onclick="chkLayer(this,\''+name+'\')" ></td>'+
'</tr>'+
'<tr><td valign = bottom>'+
...
'</td><td width="8" bgcolor="F9FCDD" ></td></tr><tr height="1"><td width="1" ><img src="/images/attach2/b_left.gif"width=8 height=8 ></td><td  bgcolor="F9FCDD"></td>	<td width="1" ><img src="/images/attach2/b_right.gif"width=8 height=8 ></td></tr>'+
...
'<td style="font-size:11px"  ><img src="/f.php?text='+icon+title+'&font=yg330&size2=10&bg=f6f6f6&color=#5B5B5B&mk_btn=Y" align=absmiddle>'+ne_img+'<input type="checkbox" name="'+name+'(str_lock)" onfocus="this.blur()"  style="cursor: hand;z-index:100000;width:10px;height:10px" align=absmiddle '+chk+' onclick="chkLayer(this,\''+name+'\')"></td>'+
'</tr>'+
'<tr><td valign = bottom>'+
...
'<table cellpadding="0" cellspacing="0" border="0" style="table-layout:fixed;position:absolute;"><tr>'+
'<td style="font-size:11px"  ><img src="/f.php?text='+icon+title+'&font=yg330&size2=10&bg=F1EFFA&color=#5B5B5B&mk_btn=Y"  align=absmiddle>'+ne_img+'<input type="checkbox" name="'+name+'(str_lock)" onfocus="this.blur()" style="cursor: hand;z-index:100000;width:10px;height:10px" align=absmiddle '+chk+' onclick="chkLayer(this,\''+name+'\')" ></td>'+
'</tr>'+
'<tr><td valign = bottom>'+
...
//$(id).disabled = "true";
	...
	function appendPanel(layer, name, title, width, avail_layer, avail_width, removable, is_part, bg, h,type,line,max,len,line_c,subject,img_w,img_h,idx,Ptop,Pleft,Pright,Pbottom,bbs_title,bbs_code,str_lock) {
		...
		if(id != "main_layout"){
		$(id).style.backgroundImage='url(/f.php?text='+layno+'&font=yg330&size2=10&bg=82848D&color=FFFF00)';
	...
	function resizeing(id,setup ,lock){ 
		...
		$('help').innerHTML = '<img src = "/f.php?text=마우스로 컨텐츠를 원하는 위치에 드래그 하십시요.&font=yg330&size2=10&bg=898B93&color=FFF82C&mk_btn=Y"><br><img src = "/f.php?text=레이아웃영역을 마우스로 드래그하면 영역 사이즈를 변경할 수 있습니다.&font=yg330&size2=10&bg=898B93&color=FFFFFF&mk_btn=Y">';
...
if(lock != "y") {
	...
	$('help').innerHTML = '<img src = "/f.php?text=마우스로 레이아웃 영역을 배치합니다.&font=yg330&size2=10&bg=898B93&color=FFF82C&mk_btn=Y"><br><img src = "/f.php?text=원하시는 위치로 각각의 레이아웃을 이동하세요.&font=yg330&size2=10&bg=898B93&color=FFFFFF&mk_btn=Y">';
