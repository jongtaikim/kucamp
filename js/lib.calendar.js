...
function Calendar(obj) {														// jucke
	...
	minical.style.pixelTop = getRealOffsetTop(obj) + obj.offsetHeight;
	minical.style.pixelLeft = getRealOffsetLeft(obj);

	minical.style.display = (minical.style.display == "block") ? "none" : "block";

	// 숨김보여줌
	toggleObject((minical.style.display == "block") ? 0 : 1);
...
function doOver() {																// 마우스가 칼렌다위에 있으면
	...
	clearTimeout(stime);													// Clear
}

function doClick() {															// 날자를 선택하였을 경우
	cal_Day = event.srcElement.title;
	event.srcElement.style.borderColor = "red";							// 테두리 색을 빨간색으로
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma;'>"
			...
			+ "<TR ALIGN=CENTER BGCOLOR=ThreedFace STYLE='color:black;font-weight:bold;'><TD>일</TD><TD>월</TD><TD>화</TD><TD>수</TD><TD>목</TD><TD>금</TD><TD>토</TD></TR>";
...
/////////////////////////////////////////////////////////////////////////
//메뉴를 클릭했을때 레이어를 덮을만한것을 임시로 숨겨주고 표시해주는 함수
