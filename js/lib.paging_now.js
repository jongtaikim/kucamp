...
Paging = function(total) {
	...
	this.calculate = function() {
		...
		this.currentPage = this.config.re_page;
	...
	this.toString = function() {
		...
		if (this.config.showFirstLast) {
			...
			firstBtn = firstBtn.link(this.config.query+"&"+this.config.pageVariable+"=1");

			if (this.config.lastIcon) lastBtn = '<img src="'+this.config.lastIcon+'" border="0" align="absmiddle" alt="마지막" />';
			else lastBtn = '[마지막]'; //'☞';
			lastBtn = lastBtn.link(this.config.query+"&"+this.config.pageVariable+"="+this.totalPage);
		...
		if (this.currentPage > this.config.pagePerView) {
			prevBtn = prevBtn.link(this.config.query+"&"+this.config.pageVariable+"="+this.prevPage);
		...
		for(var i = ss; i<se; i++) {
			...
			if (i == this.currentPage) {
				...
				_btn = '<a href="'+this.config.query+"&"+this.config.pageVariable+"="+i+'" style="'+this.config.otherPageStyle+'">'+pageText+'</a>'
			}
		...
		if (this.totalPage > this.nextPage) {
			nextBtn = nextBtn.link(this.config.query+"&"+this.config.pageVariable+"="+this.nextPage);
		}
