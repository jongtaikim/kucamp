...
switch ($mode) {
	...
	$sql = "select max(num_ccode)+1 from $table2 where num_oid = '$_OID'    and  LENGTH(NUM_CCODE)=2 ";
	$max_cate = $DB -> sqlFetchOne($sql);
	if(!$max_cate) {
		if($types ==1) $max_cate =10; else $max_cate =20; 
	}


	$sql = "select max(num_step)+1 from $table2 where num_oid = '$_OID'    and  LENGTH(NUM_CCODE)=2 ";
	...
	$sql = "INSERT INTO ". $table2." (
	 ...
	 case "del":
	  ...
	  $sql = "UPDATE ".$table." SET num_ccode=0 WHERE num_oid=$_OID and num_ccode = '".$cate."'    and  LENGTH(NUM_CCODE)=2";
	 
	  if($DB->query($sql)){
		  $DB->commit();
	  }
	 
	$sql = "delete from ".$table2." WHERE num_oid=$_OID and num_ccode = '".$cate."'    and  LENGTH(NUM_CCODE)=2";
	 if($DB->query($sql)){
		 $DB->commit();
		echo "del";
		exit;
	 ...
	 break;
	  ...
	  $str_title_ = iconv("UTF-8","euc-kr",$str_title);
	  $sql = "UPDATE ".$table2." SET str_title='".$str_title_."' WHERE num_oid=$_OID and num_ccode = '".$cate."'    and  LENGTH(NUM_CCODE)=2";
