...
switch ($REQUEST_METHOD) {
	...
	case "POST":
	 ...
	 for($ii=0; $ii<count($cates); $ii++) {
		$iia = $ii + 1;
		 if($mode=="reset"){
			 $sql = "UPDATE ".$table2." SET num_step=".$cates[$ii]." WHERE num_oid=$_OID   and  LENGTH(NUM_CCODE)=2 and num_ccode = '".$cates[$ii]."'";
		 }else{
			$sql = "UPDATE ".$table2." SET num_step=".$iia." WHERE num_oid=$_OID   and  LENGTH(NUM_CCODE)=2 and num_ccode = '".$cates[$ii]."'";
		 }
		 if($DB->query($sql)){
		 $DB->commit();
