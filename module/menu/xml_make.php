...
//if(!is_file($cache_file)) {
	...
	if($data = $DB->sqlFetchAll($sql)) {
		...
		$data[$ii]['str_link'] = str_replace("&","&amp;",$data[$ii]['str_link']);
		...
		$tabidx++;
			...
			if($data[$ii]['MENUITEM'] = $DB->sqlFetchAll($sql)) {
				...
				for($i=0; $i<count($data[$ii]['MENUITEM']); $i++) {
						...
						$data[$ii]['MENUITEM'][$i]['str_link'] = str_replace("&","&amp;",$data[$ii]['MENUITEM'][$i]['str_link']);
   ...
   // echo '<'.'?xml version="1.0" encoding="euc-kr"?'.'>';
	...
	$content = '<'.'?xml version="1.0" encoding="euc-kr"?'.'>'.$tpl->fetch('CONTENT');
