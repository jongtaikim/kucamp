...
if(THEME == "TYPE3"){
	...
	$cache_file = 'hosts/'.HOST.'/menu_2.xml';
}
...
if(!is_file($cache_file)) {
	...
	if(THEME == "TYPE3"){
		...
		$sql = "SELECT * FROM ".TAB_MENU." WHERE num_oid=$_OID AND LENGTH(num_cate)=3 AND num_view=1 ORDER BY num_step";
	}
	...
	$remoteFile = $cache_file;
	if(is_file($remoteFile)) exec("cp -R ".$remoteFile." ".$remoteFile.".backup");
	unlink($remoteFile);
