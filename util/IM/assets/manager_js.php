 ...
 */
	...
	init = function () 
	{
		...
		if (param) 
		{
			...
			f = param["f_url"];
			thumbfile = f.match(/[\/|\\]([^\\\/]+)$/);
			document.getElementById("f_thumb_url").value = thumbfile[1];
	...
	function onOK() 
	{
		...
		var fields = ["f_url", "f_alt", "f_align", "f_border", "f_horiz", "f_vert", "f_height", "f_width","f_file","f_insert","f_thumb_url","f_style","f_class"];
		...
		for (var i in fields) 
		{
			...
			else if (id == "f_thumb_url" && el.value.indexOf('://') < 0 ) {
				tmp =  el.value;
				rexp = /[\/|\\]([^\\\/]+)$/;
				param[id] = makeURL(base_url, tmp.replace(rexp, '/' + thumbdir + '/' + thumbprefix + "$1"));	
			} else
				param[id] = el.value;
		}
		__dlg_close(param);
	...
	function doUpload() 
	{
		...
		var uploadForm = document.getElementById('uploadForm');
		if(uploadForm)
			showMessage('<?php _e("Uploading","ImageManager"); ?>');
	}
	...
	function newFolder() 
	{
		...
		Dialog("newfolder.php", function(param) 
		{
			...
			else
			{
				...
				if(folder == thumbdir)
				{
					alert('<?php _e("Invalid folder name, please choose another folder name.","ImageManager"); ?>');
					return false;
				}
