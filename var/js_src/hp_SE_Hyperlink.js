...
nhn.husky.SE_Hyperlink = $Class({
	...
	$ON_MSG_APP_READY : function(){
		this.oApp.exec("REGISTER_HOTKEY", ["ctrl+k", "NCC_TOGGLE_HYPERLINK_LAYER", []]);
		
		this.oApp.registerBrowserEvent(this.oBtnConfirm, "mousedown", "NCC_APPLY_HYPERLINK");
		this.oApp.registerBrowserEvent(this.oBtnCancel, "mousedown", "HIDE_ACTIVE_LAYER");

		this.oApp.registerBrowserEvent(this.oLinkInput, "keydown", "EVENT_NCC_KEYDOWN");
		
		this.oApp.exec("REGISTER_UI_EVENT", ["hyperlink", "click", "NCC_TOGGLE_HYPERLINK_LAYER"]);
	},
	
	$ON_NCC_TOGGLE_HYPERLINK_LAYER : function(){
		if(this.oHyperlinkLayer.style.display == "none"){
			// hotkey may close the layer right away so delay here
			this.oApp.delayedExec("NCC_SHOW_HYPERLINK_LAYER", [this.oHyperlinkLayer], 0);
		}else{
			this.oApp.exec("SHOW_TOOLBAR_ACTIVE_LAYER", [this.oHyperlinkLayer]);
		}
	},
	
	$ON_NCC_SHOW_HYPERLINK_LAYER : function(){
	...
	$ON_NCC_APPLY_HYPERLINK : function(){
	...
	$ON_EVENT_NCC_KEYDOWN : function(oEvent){
		if (oEvent.key().enter){
			this.oApp.exec("NCC_APPLY_HYPERLINK");
			oEvent.stop();
		}
	}
});
