...
class FileHost {
	...
	function set_host($host='') {
		...
		$this->account = array('host'=>$this->host,'user'=>$file_ftp_conf['user'],'pass'=>$file_ftp_conf['pass'],'root_dir'=>$file_ftp_conf['root_dir']);
		$this->file_dir = $this->root_dir.'/hosts/'.HOST.'/'.$this->sect;
...
//2008-05-10 종태 임시로 업로드 처리한다.
	function upload_process_tmp($timestamp,$main,$ym_dir = false) {
		...
		if(!$this->ftp->chdir($this->file_dir.'/'.$ym_dir)) {
			...
			if(!$this->ftp->chdir($this->file_dir)) {
				$this->ftp->chdir($this->root_dir.'/hosts/'.HOST);
				if(!$this->ftp->mkdir($this->sect)){
        ...
        if($this->ftp->rename($item['filename'],$target_path)) {
					...
					$this->ftp->chmod($this->file_dir."/".date("Ym"),"777");
/*
					//2008-04-17 종태 이미지 체크 
					if($ext == "jpg" || $ext == "gif" || $ext == "png"){
...
*/
	...
	function make_thumb($filename, $width = 100) {
		if(!$width) $width = 100;
		WebApp::import('HTTP');
		$HTTP = HTTP::Connection('http://'.$this->host.'/index_file.php');
	...
	// 썸네일 URL
	function get_thumb_url($filename, $width,$mcode,$serial) {
		if(!$width) $width = 100;
		
		if(!is_file(_DOC_ROOT."/".'/hosts/'.HOST."/".$this->sect."/".$filename."_100") && is_file(_DOC_ROOT."/".'/hosts/'.HOST."/".$this->sect."/".$filename)){

		$this->GDImageResize(_DOC_ROOT."/".'/hosts/'.HOST."/".$this->sect."/".$filename , _DOC_ROOT."/".'/hosts/'.HOST."/".$this->sect."/".$filename."_100" , "100", "75");
		
		}

		if(is_file(_DOC_ROOT."/hosts/".HOST."/menu/board_img/menu.".$mcode.".".$serial."_100")){
		return "/hosts/".HOST."/menu/board_img/menu.".$mcode.".".$serial."_100";
		}else{
		
		return '/hosts/'.HOST."/".$this->sect."/".$filename."_100";
		}
	}
	...
	function get_files_info($main) {
		...
		if($data = $DB->sqlFetchAll($sql)) {
			...
			for($i=0,$cnt=count($data);$i<$cnt;$i++) {
				...
				if($data[$i]['num_ta'] && $data[$i]['num_ta'] !="none") {
					
					//2008-03-07 종태 타사이트 첨부파일 가져왔을때 처리
					$data[$i]['file_url'] = 'http://'.$_SERVER[HTTP_HOST].'/data/hosts/'.HOST.'/'.$data[$i]['num_ta']."/".$data[$i]['str_refile'];
					
					$data[$i]['real_url'] = 
					'http://'.$_SERVER[HTTP_HOST].'/data/hosts/'.HOST.'/'.$data[$i]['num_ta']."/".$data[$i]['str_refile'];
					$total_size += $data[$i]['num_size'];
					
					//2008-12-29 현민 에디터에서 삽입된 이미지는 제거하고 첨부파일 보여줌.
					$data2[$ii] = $data[$i];
					$ii++;
				}else{
					
					if(!strstr($data[$i]['str_refile'],"/data/hosts/")){
			...
			if($data[$i]['num_ta'] = "none") unset($data[$i]);
				
				
			}
			$data2['total_size'] = $total_size;
		}
		return $data2;
		//return $sql;
	}
	...
	function get_real_url($refile) {
		return 'http://'.$this->host.'/hosts/'.HOST.'/'.$this->sect.'/'.$refile;
	}
