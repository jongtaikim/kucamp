...
class Display extends Template_ {
	...
	function setLayout($conf) {
	 global $nolayout,$_OID,$mcode;
		if(!$conf) $conf ="@sub";
		if($nolayout) $conf ="ajax";
		

		if(!$_SESSION[$mcode][layout] && $mcode  ){
		$DB = &WebApp::singleton('DB');
		$_SESSION[$mcode][layout] = $DB->sqlFetchOne("SELECT str_layout FROM ".TAB_MENU." WHERE NUM_OID=$_OID AND NUM_MCODE=$mcode");
		$_SESSION[$mcode][layout] = str_replace("@","",$_SESSION[$mcode][layout]);
		}
		if($_SESSION[$mcode][layout] && strstr($conf,"sub")){
			$conf = "@".$_SESSION[$mcode][layout];
		}
		...
		//잠시 막기
		if(date("YmdH") <='2016052018'){
		//$layers[LAYOUT] = "@/layout_main_11.htm";
		}
	   ...
	   if ($this->layout == 'xml') {
			...
			$this->define('LAYOUT','html/layout_blank');
		} elseif ($this->layout == 'frameset' || $this->layout == 'blank') {
			$this->define('LAYOUT','html/blank.htm');
	...
	*/
    ...
    function AttachConf($sect = false, $flag = false, $THEME = THEME, $HOST = HOST) {
        static $ATT_CONF;
        if($flag || !$ATT_CONF) {
            $ATT_CONF = @parse_ini_file('theme/'.$THEME.'/conf/attach.conf.php',true);
            if (!is_array($local_conf = @parse_ini_file('hosts/'.$HOST.'/conf/'._CSS.'.attach.conf.php',true))) $local_conf = array();
            $ATT_CONF = Display::array_merge_recursive2($ATT_CONF,$local_conf);
            foreach($ATT_CONF as $key => $_conf) {
                eval("\$ATT_CONF['$key']['file'] = ".$_conf['file'].';');
                eval("\$ATT_CONF['$key']['avail_layer'] = ".$_conf['avail_layer'].';');
