...
class DB2 {
	...
	* 
	*/
	function DB2($dsn='default') {
		$thisObj = &$this;
		$thisObj = (object)DB2::dsn($dsn);
...
//		return $varname;
	...
	function dsn($conf) {
		...
		$ret = &DB2::singleton($info['dbms'],$info['host'],$info['user'],$info['pass'],$info['db']);
		return $ret;
	}
...
//		$info['db'] = str_replace('/','',$info['path']);
		...
		$ret = &DB2::singleton($info['scheme'],$info['host'],$info['user'],$info['pass'],$info['db']);
		return $ret;
	}
	...
	function &singleton($scheme,$host,$user,$pass,$db) {
		...
		if (is_object($instance[$signature])) {
			...
			require_once $_SERVER["DOCUMENT_ROOT"]."/lib/DB/$scheme.php";
	...
	function sqlsafe($query) {
		...
		if ($cmd == 'UPDATE' || $cmd == 'DELETE') {
			if (!in_array('WHERE',$parts)) {
				WebApp::alert("SQL경고: WHERE 절이 필요합니다.\n$query");
				return;
			}
		}
		return true;
