...
class DB_Oracle extends DB {
	...
	//function DB_Oracle($host='',$user='ewutadmin',$pass='qptmxmdldnt',$sid='ewut') {
	function DB_Oracle($host='',$user='ewutadmin',$pass='ewut0673',$sid='ewut') {
		// XXX : 호스트정보는 버림
	...
	function query($sql,$mode=OCI_DEFAULT) {
		if (!$this->sqlsafe($sql)) return false;		
		$this->stmt = OciParse($this->conn,$sql);
		return $this->exec($mode);
	}


	function query_clob($sql,&$str_text,$mode=OCI_DEFAULT) {
		if (!$this->sqlsafe($sql)) return false;		
		$this->stmt = OciParse($this->conn,$sql);
		$this->bindByName(":str_text",&$str_text,-1);		
		return $this->exec($mode);
	}
