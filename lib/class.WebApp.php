...
function ImgFindUpload($str_text){ //외부이미지 넣기
	...
	$code = $FH->code;
		...
		for($ii=0; $ii<count($tmp_img_list); $ii++) {
			...
			if(!strstr($tmp_img_list[$ii],$_SERVER[HTTP_HOST]) &&  !strstr($tmp_img_list[$ii],DOMAIN_) &&  !strstr($tmp_img_list[$ii],"FILE_HOST") && strstr($tmp_img_list[$ii],"http://")){
		...
		$file_name = $mk.$ii.".gif";
			...
			//$filesize = ftp_size($FTP->conn,$target_path);
					...
					$size=GetImageSize($tmp_file[$ii]); 
					$FTP->chmod($FH->file_dir."/".date("Ym"),"777");											
					if($size[0] > 800) {

					$FH->GDImageResize($tmp_file[$ii] , $target_path , "800", "600");
					
					}else{
					$FH->GDImageResize($tmp_file[$ii], $target_path);
					}
...
function ImgFindUploadUrl($str_text){ //외부이미지 넣기
	global $_OID, $sess_id, $DB, $FH,$serial;
	...
	$code = $FH->code;
		...
		for($ii=0; $ii<count($tmp_img_list); $ii++) {
			...
			$ch = curl_init($tmp_img_list[$ii]);
			$mk = mktime();
			$tmp_file[$ii] = $FH->file_dir."/board_img/".$mk."_".$ii.".gif";
		...
		$file_name = $mk.$ii.".gif";
			...
			WebApp::saveThumbImg($target_path,$target_path."_100",106,74,100, 1, "");
			...
			$r_file = $FH->get_real_url($refile[$ii]);
			/*if($ii == 0){
				$str_text = '<img src="'.$r_file.'" style="display:none">'.$str_text;
			}*/
...
function ImgChaneDe($content, $main="-2"){ // 새로운 에디터 이미지 쓰레기 처리 이현민 대리
	global $_OID, $sess_id, $DB, $FH,$serial,$id ;
	...
	$s = preg_match_all("/<embed\s+.*?src=[\"\']([^\"\']+)[\"\'\s][^>]*>/is", $content, $m2); 
	...
	for($ii=0; $ii<count($tmp_img_list); $ii++) {
		if((strstr($tmp_img_list[$ii],"/hosts/".HOST."/board_tmp/") && $sess_id) || 
			(strstr($tmp_img_list[$ii],$_COOKIE['PHPSESSID']))){

			//에디터삽입이미지처리
			if(strstr($tmp_img_list[$ii],$_COOKIE['PHPSESSID'])){
				//여긴 스마트에디터 타입
				//http://isch_file.ezsol.kr/tmp_upload/80853186e4c88b12f821660a0d3d8447/1237785228/10.jpg
				$deimg = urldecode($tmp_img_list[$ii]);
				$fpath = $FH->root_dir.str_replace("http://".$_SERVER[HTTP_HOST],"",$deimg);
			}else{
				//여긴 editor_new 타입
			...
			$rfname = $ym_dir.'/'.$code.'.'.$main.'.'.$new_serial.'.'.mktime()."_".$i.'.'.$ftype;
			$sql1 = "
					INSERT INTO ".TAB_FILES." (
						NUM_OID,STR_SECT,STR_CODE,NUM_MAIN,NUM_SERIAL,STR_UPFILE,STR_REFILE,STR_FTYPE,NUM_SIZE,NUM_TA
					) VALUES (
						$_OID,'thumb','$code','$main',$new_serial,'$fname','$rfname','$ftype','$fsize','none'
					)";
			...
			exec("mv ".$fpath." ".$FH->file_dir.'/'.$rfname);
			...
			WebApp::saveThumbImg($FH->file_dir.'/'.$rfname,$FH->file_dir.'/'.$rfname."_100",106,74,100, 1, "");
		
			if($FH->sect =="menu"){
			if(!$serial) $serial = $id;
			$sql = "UPDATE TAB_BOARD	SET  str_thumb='".$rfname."'	WHERE	num_oid = ".$FH->oid." and num_mcode=".$code." AND num_serial=".$serial."";
			
			$DB->query($sql); 
			$DB->commit();

			$_SESSION['get_thumb_filename'] = $rfname;
	


			}else if(strstr($code,"news.")){
			$code_e = explode(".",$code);
			if(!$serial) $serial = $id;
			$sql = "UPDATE TAB_MAIN_BOARD	SET  str_thumb='".$rfname."'	WHERE	num_oid = ".$FH->oid." and str_code='".$code_e[1]."' AND num_serial=".$serial."";
	
			$DB->query($sql); 
			$DB->commit();
			}else if($FH->sect =="party"){
			$code_e = explode(".",$code);
			if(!$serial) $serial = $id;
			$sql = "UPDATE TAB_PARTY_BOARD	SET  str_thumb='".$rfname."'	WHERE	num_oid = ".$FH->oid." and num_pcode='".$code_e[0]."' and num_mcode='".$code_e[1]."' AND num_serial=".$serial."";
	...
	$content = str_replace($FH->host,$_SERVER[HTTP_HOST],$content);

	// /board_tmp/ 에 하루가 지난 폴더가 있으면 여기서 삭제하자~

	$del_dir = _DOC_ROOT."/hosts/".HOST."/board_tmp/".date("Ymd", mktime(0,0,0,date("m"),date("d")-2,date("Y")))."/";
	if (is_dir($del_dir)) { 
	...
	$content = str_replace("</WA:APPLET>","</wa:applet>",$content);
	$content = str_replace("<?XML:NAMESPACE PREFIX = WA />","",$content);
...
function saveThumbImg($readFilename,$saveFilename,$w=0,$h=0,$quality=100, $sizetype1=1, $sizetype2) {
	...
	elseif($image_type==2) 	$result=imagejpeg($img_thumb,$saveFilename,"100");
...
function maxSerial($table,$cum,$query){
$DB = &WebApp::singleton('DB');
$sql = "select max($cum)+1 from $table where num_oid = "._OID." $query";
...
function OIDUploadFileSize(){
	...
	if(!$num_disk) $num_disk=5*1024*1024*1024;
	//수치가 없는경우 50mb로 제한
...
//2010-01-13 종태  RSS 관련 function  추가함
...
function RSS_reader($count,$url,$encode = "EUC-KR"){ 
	...
	$channel = WebApp::parseTag("channel",WebApp::getRSS($url,$encode)); 
    // 인코딩 변환 
    ...
    // 제목 처리 
    $siteTitle = WebApp::parseTag("title",$item[0]); 
    $siteLink = WebApp::parseTag("link",$item[0]); 
    $siteDescription = WebApp::parseTag("description",$item[0]); 
    ...
    for($i=1;$i<=$count;$i++){ 
        if(!isset($item["$i"])) break; 
				...
				$title[$ii] = WebApp::parseTag("title",$item[$i]); 
				$data[$ii][str_title] = $title[$ii];
				$data[$ii][str_text] =  str_replace("&","&amp;",WebApp::parseTag("description",$item[$i]));
				$data[$ii][thumb_url] =  str_replace("&","&amp;",WebApp::parseTag("thumb_url",$item[$i]));
				$data[$ii][str_link] =  str_replace("&","&amp;",WebApp::parseTag("link",$item[$i]));
				$data[$ii][str_url] =  str_replace("&","&amp;",WebApp::parseTag("link",$item[$i]));
				$data[$ii][str_dis_url] =  WebApp::hstrCut(str_replace("&","&amp;",WebApp::parseTag("link",$item[$i])),"60","..");
...
// 보여지는 부분 
function RSS_query($count,$url,$encode = "EUC-KR"){ 
    global $mcode,$DB,$_conf;
	$channel = WebApp::parseTag("channel",WebApp::getRSS($url,$encode)); 
    // 인코딩 변환 
    ...
    $siteLink = WebApp::parseTag("link",$item[0]); 
    $siteDescription = WebApp::parseTag("description",$item[0]); 
	$total = WebApp::parseTag("total",$item[0]); 
...
function pointUpdate($cum='board',$point='0',$keys='up',$userid){
global $DB,$_OID,$mcode;
	...
	if($userid){
		...
		if($keys == "up"){
			$qsql = " num_".$cum."_cnt = num_".$cum."_cnt + $point ";
		}else{
			$qsql = " num_".$cum."_cnt = num_".$cum."_cnt - $point ";
		}
	
		$sql = "UPDATE ".TAB_POINT." SET $qsql WHERE num_oid=$_OID AND str_user_id='".$userid."'";
		if($DB->query($sql)){
		$DB->commit();
		return true;
		}
...
function log_txt($msg,$logfile){
global  $mcode;
	if(!$logfile) return;
	
	$FTP = &WebApp::singleton('FtpClient',WebApp::getConf('account'));
	$FTP->mkdir(_DOC_ROOT.'/hosts/'.HOST.'/log');
	$FTP->chmod(_DOC_ROOT.'/hosts/'.HOST.'/log',777);
	$logfile = _DOC_ROOT.'/hosts/'.HOST.'/log/'.$logfile;
	
	$log = file_get_contents($logfile);
	$log_b = date("Y.m.d H:i:s")."[".$_SESSION[NAME]."|".$_SESSION[USERID]."(".$_SERVER["REMOTE_ADDR"].")"."] : ".$msg."\n-------------------------------------------------------------\n".$log;
	if($FTP->put_string($log_b,$logfile)){
		return true;
	}
}
...
function memberPoint($type,$chk,$str_id){
	global $_OID;
	if($str_id){
		...
		$plus_point = "num_login_point";
			$sql = "UPDATE ".TAB_MEMBER." SET $plus_point = $plus_point  $sum 1  WHERE num_oid="._OID." AND str_id='".$str_id."'";
			$DB->query($sql);
			$DB->commit();
		}
		...
		$row[num_point_total] = $row[num_login_point] * $chw[num_login_point];
		$row[num_point_total] = $row[num_point_total] + $row[num_board_point] * $chw[num_board_point];
		$row[num_point_total] = $row[num_point_total] + $row[num_commint_point] * $chw[num_commint_point];
		$row[num_point_total] = $row[num_point_total] + $row[num_repaly_point] * $chw[num_repaly_point];
		
	

		 $sql = "UPDATE ".TAB_MEMBER." SET num_point_total='".$row[num_point_total]."' WHERE num_oid='"._OID."' and str_id = '".$str_id."' ";
		
		 if($DB->query($sql)){
			 $DB->commit();
		 }
		
		return $row[num_point_total];
	}
}
